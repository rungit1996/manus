...
class Browser(Protocol):
    ...
    async def input(
        ...
        """传递文本+回车标识+索引/xy坐标实现在网页输出框中输入对应的内容"""
        ...
    ...
    async def scroll_up(self, to_top: Optional[bool] = None) -> ToolResult:
        """向上滚动浏览器，如果没传递 to_top = True 则向上滚动一页，否则直接滚动到最顶部"""
        ...

    async def scroll_down(self, to_down: Optional[bool] = None) -> ToolResult:
        """向下滚动浏览器，如果没传递 to_down = True 则向下滚动一页，否则直接滚动到最底部"""
        ...

    async def screenshot(self, full_page: Optional[bool] = None) -> bytes:
        """对当前浏览器页面进行截图，传递 full_page = True 时会截图整个页面"""
        ...

    async def console_exec(self, javascript: str) -> ToolResult:
        """传递对应的 js 脚本，在浏览器的当前页面控制台执行"""
        ...
