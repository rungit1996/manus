...
2. 定义一个装饰器，被该装饰器装饰的方法会填充_tool_name、_tool_description、_tool_schema、_required 属性
3. 工具类可以通过 get_tools 快速获取基于缓存的 schema 参数信息，这样 LLM 就可以便捷调用
4. LLM 生成的内容可能会存在幻觉，在调用工具前需要筛选出 LLM 生成参数中符合工具的相关数据
"""
import inspect
...
class BaseTool:
    ...
    def _filter_parameters(cls, method: Callable, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        """传递 method + kwargs 并过滤参数，使其符合 method 参数的要求，因为 LLM 输出的内容可能有幻觉"""
    ...
    def get_tools(self) -> List[Dict[str, Any]]:
        """获取所有已注册的工具列表 schema 信息，用于 LLM 绑定工具"""
        # 1. 判断缓存是否存在
    ...
    async def invoke(self, tool_name: str, **kwargs) -> ToolResult:
        """根据传递的工具名 + kwargs 调用指定工具并获取指定结果"""
        # 1. 循环遍历工具集的所有方法
        for _, method in inspect.getmembers(self, inspect.ismethod):
            # 2. 判断对应的方法是否存在 _tool_name
            if hasattr(method, "_tool_name") and getattr(method, "_tool_name") == tool_name:
                # 3. 筛选传递的 kwargs 参数保留 method 对应的参数，多余的剔除
