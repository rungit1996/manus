...
class FileTool(BaseModel):
    ...
    @tool(
        ...
        parameters={
            ...
            "end_line": {
                "type": "integer",
                "description": "（可选）结束行号，不包括该行。",
            },
            "sudo": {
                "type": "boolean",
                "description": "（可选）是否使用 root 权限。",
            },
            "max_length": {
                "type": "integer",
    ...
    @tool(
        ...
        parameters={
            ...
            "trailing_newline": {
                "type": "boolean",
                "description": "（可选）是否添加后置换行符，在内容末尾。",
            },
            "sudo": {
                "type": "boolean",
    ...
    async def find_files(
        ...
        """传递目录的绝对路径和通配符，在指定目录中根据名称模式查找文件"""
    ...
    @tool(
        ...
        parameters={
            "dir_path": {
                "type": "string",
                "description": "要列出的文件列表目录的绝对路径。",
            },
        },
        required=["dir_path"]
    )
    async def list_files(self, dir_path: str) -> ToolResult:
        """传入目录的绝对路径，列出该目录下的文件列表信息"""
