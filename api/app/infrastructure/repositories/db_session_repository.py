...
class DBSessionRepository(SessionRepository):
    ...
    async def get_file_by_path(self, session_id: str, file_path: str) -> Optional[File]:
        """根据文件路径获取文件信息"""
        # 1. 构建 sql 查询文件列表
        ...
        # 3. 遍历查找数据，返回文件信息
        for file in files:
            if file.get("filepath", "") == file_path:
                return File(**file)
    ...
    async def get_memory(self, session_id: str, agent_name: str) -> Memory:
        ...
        # 2. 如果记忆存在则转换成 Domain 返回
        if memory_data:
            return Memory(**memory_data)

        # 3. 如果记忆不存在则创建一个空记忆后返回
