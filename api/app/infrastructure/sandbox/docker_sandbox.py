...
class DockerSandbox(Sandbox):
    ...
    async def ensure_sandbox(self) -> None:
        """确保沙箱一定存在/服务全部开启了才执行后续步骤"""
        # 1. 定义最大重试次数+重试间隔
        max_retries = 30
        retry_interval = 2

        # 2. 循环请求获取 supervisor 状态并判断服务是否正常
        for attempt in range(max_retries):
            try:
                ...
                if not tool_result.success:
                    logger.warning(f"Supervisor 进程状态监测失败：{tool_result.message}")
                ...
                if not services:
                    logger.warning(f"Supervisor 进程中未发现任何服务")
                    await asyncio.sleep(retry_interval)
                    continue
                ...
                if all_running:
                    ...
                    logger.info(f"正在等待 Sandbox Supervisor 进程服务运行，还未运行的服务列表：{non_running_services}")
                    await asyncio.sleep(retry_interval)
            except Exception as e:
