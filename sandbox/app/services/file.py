...
class FileService:
    ...
    async def write_file(
        ...
        try:
            ...
            if sudo:
                ...
                # 5. 创建一个内部函数使用 asyncio 创建新线程写入数据
                def async_write_temp_file() -> int:
                    with open(temp_file, "w", encoding="utf-8") as f:
                        f.write(content)
                    return len(content.encode("utf-8"))

                # 6. 使用 asyncio 创建子线程并写入
    ...
    async def replace_in_file(
        ...
        # 2. 计算 old_str 出现的次数，只要次数大于 0 才需要替换
    ...
    async def search_in_file(self, filepath: str, regex: str, sudo: bool = False) -> FileSearchResult:
        """根据传递的文件路径+匹配规则，查询文件内符合的内容"""
        # 1. 调用服务获取对应的服务内容
        file_read_result = await self.read_file(filepath=filepath, sudo=sudo, max_length=None)
        content = file_read_result.content
        ...
        # 4. 创建一个异步函数，使用子线程方式执行避免长时间 io 阻塞
        def async_matches():
    ...
    async def find_files(cls, dir_path: str, glob_pattern: str) -> FileFindResult:
        """根据传递的文件夹路径和+glob 规则查找文件列表"""
        # 1. 检测下传递进来的目录是否存在
        if not os.path.isdir(dir_path):
            raise NotFoundException(f"目录不存在或不是文件夹：{dir_path}")
    ...
    async def delete_file(self, filepath: str) -> FileDeleteResult:
        ...
        try:
            ...
            logger.error(f"删除 {filepath} 文件失败：{str(e)}")
            raise AppException(f"删除 {filepath} 文件失败：{str(e)}")
