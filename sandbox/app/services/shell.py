...
class ShellService:
    ...
    async def _create_process(cls, exec_dir: str, command: str) -> asyncio.subprocess.Process:
        ...
        # 1. ubuntu 系统统一使用 "/bin/bash" 这个解释器
        logger.debug(f"在目录 {exec_dir} 下使用命令 {command} 创建一个子进程")
        shell_exec = "/bin/bash"

        # 2. 创建一个系统级的子进程用来执行 shell 命令
    ...
    async def wait_process(self, session_id: str, seconds: Optional[int] = None) -> ShellWaitResult:
        """传递会话id+时间，等待子进程结束"""
        # 1. 判断下传递的会话是否存在
        logger.debug(f"正在 Shell 会话中等待进程：{session_id}，超时：{seconds}s")
        ...
        try:
            ...
            logger.error(f"Shell 会话进程等待过程出错：{str(e)}")
            raise AppException(f"Shell 会话进程等待过程出错：{str(e)}")
    ...
    def get_console_records(self, session_id: str) -> List[ConsoleRecord]:
        """从指定会话中获取控制台记录"""
        # 1. 判断下传递的会话是否存在
        logger.debug(f"正在获取 Shell 会话的控制台记录：{session_id}")
        if session_id not in self.active_shells:
            logger.error(f"Shell 会话不存在：{session_id}")
            raise NotFoundException(f"Shell 会话不存在：{session_id}")
    ...
    async def read_shell_output(self, session_id: str, console: bool = False) -> ShellReadResult:
        """根据传递的会话id+是否输出控制台记录获取 Shell 命令结果"""
        # 1. 判断下传递的会话是否存在
    ...
    async def exec_command(self, session_id: str, exec_dir: str, command: str) -> ShellExecuteResult:
        ...
        try:
            ...
            if session_id not in self.active_shells:
                ...
                # 6. Shell 会话已存在，则读取数据
                logger.debug(f"使用现用的 Shell 会话：{session_id}")
                shell = self.active_shells[session_id]
                ...
                if old_process.returncode is None:
                    ...
                    try:
                        ...
                        # 9, 结束旧进程出现错误后记录日志并调用 kill 强制关闭进程
                        logger.warning(f"强制终止 Shell 会话中的进程 {session_id} 失败：{str(e)}")
                        old_process.kill()
    ...
    async def write_shell_input(self, session_id: str, input_text: str, press_enter: bool) -> ShellWriteResult:
        """根据传递的数据向指定子进程写入数据"""
        # 1. 判断下传递的会话是否存在
